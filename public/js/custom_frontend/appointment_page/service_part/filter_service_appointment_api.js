/* SERVICE APPOINTMENT FILTER API */
(function () {
    ...
    app.loadAppointments = async function () {
        ...
        if (app.resetButton) app.resetButton.disabled = true;
        ...
        try {
            ...
            if (!response.ok)
                throw new Error("Failed to filter service appointments.");
            ...
            console.error("Service appointment filter error:", error);
            alert("Unable to filter service bookings. Please try again.");
        } finally {
            ...
            if (app.resetButton) app.resetButton.disabled = false;
        }
    };
})();
